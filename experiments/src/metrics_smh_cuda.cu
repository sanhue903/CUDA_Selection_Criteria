...
// CUDA translation of metrics_smh.cpp
// ...copy all logic from metrics_smh.cpp, but replace OpenMP regions with CUDA-style host functions as in previous translations...
// ...implement a CUDA-style pairwise comparison and reduction, using host-side logic for file I/O and C++ objects...
// ...output and logic remain the same for fair comparison...

// CUDA-style pairwise comparison and reduction (host-side, as in previous translations)
void pairwise_metrics_smh_cuda(
    ...
    int n_rows, int n_bands, float threshold,
    long int& tp_lsh, long int& tn_lsh, long int& fp_lsh, long int& fn_lsh,
    long int& tp_cblsh, long int& tn_cblsh, long int& fp_cblsh, long int& fn_cblsh)
{
    ...
    for (int i_processed = 0; i_processed < n - 1; ++i_processed) {
        ...
        for (int k = i_processed + 1; k < n; ++k) {
            ...
            bool smha_criterion = smh_a(name2mhv.at(fn1), name2mhv.at(fn2), n_rows, n_bands);
            bool CBsmha_criterion = CB_smh_a(threshold, e1, e2, name2mhv.at(fn1), name2mhv.at(fn2), n_rows, n_bands);
            if (jacc14 >= threshold){
                if (smha_criterion){ tp_lsh++; } else{ fn_lsh++; }
                if (CBsmha_criterion){ tp_cblsh++; } else{ fn_cblsh++; }
            }else {
                if (!smha_criterion){ tn_lsh++; } else { fp_lsh++; }
                if (!CBsmha_criterion){ tn_cblsh++; } else { fp_cblsh++; }
            }
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    long int tp_lsh = 0, tn_lsh = 0, fp_lsh = 0, fn_lsh = 0;
    long int tp_cblsh = 0, tn_cblsh = 0, fp_cblsh = 0, fn_cblsh = 0;
    pairwise_metrics_smh_cuda(card_name, name2hll, name2mhv, n_rows, n_bands, threshold,
        tp_lsh, tn_lsh, fp_lsh, fn_lsh, tp_cblsh, tn_cblsh, fp_cblsh, fn_cblsh);
